...
namespace ScenarioBot.Repository.Impl.InMemory
{
    ...
    /// </summary>
    public class MemoryStorage : ScenarioBot.Repository.Impl.InMemory.IStorage
    {
        ...
        /// <seealso cref="WriteAsync(IDictionary{string, object}, CancellationToken)"/>
        public Task<IDictionary<string, T>> ReadAsync<T>(string[] keys, CancellationToken cancellationToken)
        {
            if (keys == null)
            {
                throw new ArgumentNullException(nameof(keys));
            ...
            var storeItems = new Dictionary<string, T>(keys.Length);
            lock (_syncroot)
            {
                foreach (var key in keys)
                {
                    if (_memory.TryGetValue(key, out var state))
                    {
                        if (state != null)
                        {
                            storeItems.Add(key, state.ToObject<T>(StateJsonSerializer));
                        }
                    }
                }
            }

            return Task.FromResult<IDictionary<string, T>>(storeItems);
        }
        ...
        /// <seealso cref="DeleteAsync(string[], CancellationToken)"/>
        /// <seealso cref="ReadAsync(string[], CancellationToken)"/>
        public Task WriteAsync(Dictionary<string, object> changes, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (changes == null)
            {
